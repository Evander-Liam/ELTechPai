...
public class CommentWriteServiceImpl implements CommentWriteService {
    ...
    public Long saveComment(CommentSaveReq commentSaveReq) {
        try {
            String errorMsg = ContextAudit.scanText(commentSaveReq.getCommentContent());
            if (StringUtils.isNotBlank(errorMsg)) {
                ForumException forumException = ExceptionUtil.of(StatusEnum.COMMENT_CONTENT_VIOLATION);
                forumException.getStatus().setMsg(errorMsg);
                throw forumException;
            }
        }  catch (ForumException fe) {
            log.error("Comment's rejected with auditing! msg: {}", fe.getStatus().getMsg());
            throw fe;
        }
